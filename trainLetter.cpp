...
bool graficarFeatures(const string& data_filename) {

	Mat dataTest;
	Mat responsesTest;
	Mat dataTest_x1_norm;
	Mat dataTest_x2_norm;

	bool ok = read_num_class_data(data_filename, 2, &dataTest, &responsesTest);
	if (!ok)
		return ok;

	int nsamples_all = dataTest.rows;

	dataTest_x1_norm.create(Size(dataTest.rows, 1), CV_32F);

	dataTest_x2_norm.create(Size(dataTest.rows, 1), CV_32F);

	normalize(dataTest(Range::all(), Range(0, 1)), dataTest_x1_norm, 0.0, 255, NORM_MINMAX, CV_32F);

	normalize(dataTest(Range::all(), Range(1, 2)), dataTest_x2_norm, 0.0, 255, NORM_MINMAX, CV_32F);

	responsesTest -= 'A';

	for (int j = 0; j<dataTest.rows; j++)
		cout << responsesTest.at<int>(j);

	//predecir(model, dataTest, responsesTest);

	Mat plotFeatures = Mat::zeros(Size(256, 256), CV_8UC3);

	int a = responsesTest.at<int>(0, 0);

	for (int j = 0; j<dataTest.rows; j++) {
		if (responsesTest.at<int>(j) == 0) {
			circle(plotFeatures, Point((int)dataTest_x1_norm.at<float>(j, 0), (int)dataTest_x2_norm.at<float>(j, 0)), 3, Scalar(255, 0, 0));
		}
		else {
			circle(plotFeatures, Point((int)dataTest_x1_norm.at<float>(j, 0), (int)dataTest_x2_norm.at<float>(j, 0)), 3, Scalar(0, 0, 255));
		}
	}

	imshow("plotFeatures", plotFeatures);
	waitKey(1000);
...
int main(int argc, char *argv[])
{
	...
	string data_filename = "miletra.data";
	int method = 0;
